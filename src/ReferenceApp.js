...
const ReferenceApp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setCategory(data.category); // Устанавливаем категорию из ответа
      ...
      <div className="right-panel">
        {loading ? (
          ...
          <div className="response">
            <div className='response-close-modal-button'>
              <button className='response-modal-button' onClick={() => setIsModalOpen(true)}>
                <img className='response-close-modal' src={require('../src/icons/folder-open.png')} />
              </button>
              <button className='response-modal-button' onClick={handleCopy(response)}>
                <img className='response-close-modal' src={require('../src/icons/copy.png')} alt="Copy" />
              </button>
            </div>
            <h4>Категория: {category || 'Категория не определена'}</h4>
            <h4>Ответ:</h4>
        ...
        {alternativeResponse && (
          <div className="alternative-response">
            <div className='response-close-modal-button'>
              ...
              <button className='response-modal-button' onClick={handleCopy(alternativeResponse)}>
                <img className='response-close-modal' src={require('../src/icons/copy.png')} alt="Copy" />
              </button>
            </div>
